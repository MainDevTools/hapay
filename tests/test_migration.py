...
def main():
    ...
    # Звіряємо з ФАЙЛАМИ на диску, а не з зашитим переліком: раніше тут стояло
    # [1..11], і кожна нова міграція валила CI зміною, яка до неї не має стосунку.
    # Суть перевірки — «застосувалися всі, без дірок у нумерації», а не «їх рівно 11».
    on_disk = sorted(int(os.path.basename(f)[:4])
                     for f in glob.glob(os.path.join(migrate.MIG_DIR, "[0-9][0-9][0-9][0-9]_*.sql")))
    checks.append((f"застосовано всі міграції з диска ({len(on_disk)}: 0001…{on_disk[-1]:04d})",
                   applied == on_disk, (applied, on_disk)))
    checks.append(("нумерація міграцій без дірок",
                   on_disk == list(range(1, len(on_disk) + 1)), on_disk))
