...
- **Сховище (ухвалено, O1+T11):** **чистий PostgreSQL 16** — ship на **Neon** (free-forever); `0001`-схема §6 (звичайні таблиці, `tsvector`-FTS, append-only тригери, покривні індекси) — plain PG, Timescale не потрібен. Timescale hypertable/cagg/компресія — **deferred `0002` scale-upgrade** (Timescale Cloud/VPS), коли обсяг вимагатиме. Neon переживає ефемерні GH-прогони й дає SQL-доступ read-API. *(Відкинуті: SQLite у git #159; Turso/D1. T10 обрав Postgres, T11 — plain-PG/Neon заради нуль-вартості на валідації.)*
