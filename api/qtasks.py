...
PAGE_REPEAT_MIN = 1440     # сторінки 1..3 — раз на добу (одна доба = одна точка історії)
DEEP_REPEAT_MIN = 2880     # сторінки 4+ — раз на 2 доби (хвіст майже не рухається)
...
def seed_tasks(conn) -> int:
    ...
    for source, cfg in HTML_SOURCES.items():
        ...
        if cfg.get("sitemap"):                          # sitemap-відкриття (T20): крамниця сама
            rows.append((source, cfg["sitemap"]["url"], "sitemap", SITEMAP_REPEAT_MIN))
