...
# ── коди підтвердження (S13: verify email / reset пароля) ──────────────────────────
VERIFY_TTL_S = 24 * 3600              # лист підтвердження email — доба
RESET_TTL_S = 3600                    # скидання пароля — година (чутливіше)


def make_code() -> str:
    """6-значний числовий код (звичний UX листа). Захист від перебору — не в довжині
    коду, а в TTL + одноразовості + прив'язці + rate-limit на спроби вводу."""
    return f"{secrets.randbelow(1_000_000):06d}"


def hash_code(code: str) -> str:
    """sha256(код) для зберігання в БД. Код — швидкоплинний і одноразовий, тож простий
    хеш достатній (на відміну від пароля, якому потрібен повільний pbkdf2)."""
...
# Хеш-«пустушка» для входу з НЕІСНУЮЧИМ email. Рахується один раз на старті від
# випадкового пароля — у коді його немає, підібрати нічого.
#
# Навіщо: без нього вхід відповідав помітно швидше, коли акаунта нема, бо pbkdf2
# (600k ітерацій) просто не викликався. Заміряно на живому 2026-07-27: **245 мс
# проти 421 мс**, діапазони навіть не перетинались — тобто «чи є такий акаунт»
# читалося з таймінгу так само надійно, як із коду відповіді. У login при цьому
# СТОЯВ коментар, що захист є: `and` у Python коротко замикається, тож
# `verify_password` для неіснуючого юзера не викликався жодного разу.
