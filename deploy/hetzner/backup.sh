#!/usr/bin/env bash
# pg_dump бази ПОЗА цей сервер. Запускається systemd-таймером (radar-backup.timer).
#
# ── Навіщо це існує ─────────────────────────────────────────────────────────────
...
: "${DATABASE_URL:?нема DATABASE_URL (EnvironmentFile=/etc/radar/radar.env)}"
...
DUMP="$WORK/radar-$STAMP.dump"

echo "==> pg_dump ($STAMP)"
# -Fc: стиснутий формат + вибіркове відновлення через pg_restore
...
echo "==> перевірка цілісності (читаємо назад те, що записали)"
