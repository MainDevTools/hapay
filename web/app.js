...
/* Чіпи стрічки. З S34 стрічка показує ВСІ товари, тож «лише знижки» стало окремим
   зрізом, а не мовчазним режимом за замовчуванням: половина каталогу знижки не має,
   і ховати її означало б удавати, що її не існує. */
const BADGES = [
  {k:'', label:'Усі товари'},
  {k:'sale', label:'Лише знижки'},
...
/* Сортування за замовчуванням — «популярні моделі», а не «заявлена знижка».
   Заміряно на живому після переходу на /api/products: при sort=discount перша
   сторінка складається З САМИХ знижок (0 із 50 без знижки), тобто відкриті
   31 тисяча товарів лишались невидимими — сторінка виглядала так само, як до
   зміни. popular дає товар, який продають найбільше крамниць: для каталогу це
   і чесніше, і корисніше за «у кого найгучніший відсоток». */
let badge='', onlyDiscounts=false, sort='popular', cat='', query='', page=0;
...
function renderHeader(active, opts){
  ...
  // «Каталог», а не «Знижки»: з S34 сторінка показує всі 59 445 товарів, і стара
  // назва була б просто неправдою. Решту словника двох поверхонь зводить оператор.
  const nav = [['/', 'Головна'], ['/catalog', 'Каталог'], ['/drops', 'Подешевшало']];
