...
**Реалізована мапа видимих міток** — чинна з 2026-07-26 (`web/app.js` + `app-maui`):

| Енум (внутрішній) | Чип-фільтр | Пігулка на картці |
|---|---|---|
| `verified` | ✓ Підтверджені | 🛡 знижка перевірена |
| `verified_provisional` | (разом із `verified`) | 🛡 знижка перевірена |
| `pumped` | ⚠ Завищена стара ціна | ⚠ «стара» ціна завищена |
| `declared` | — | **нічого не кажемо** |
| `insufficient_history` | — | **нічого не кажемо** |

> ### 🔁 Зміна рішення 2026-07-26 (оператор): сайт узято ПІД застосунок
>
> Попередня мапа (нижче) трималась на T12: мітки свідомо були **вимірами, не вердиктом**
> («↓ Нижче за 30 днів», «≥ Не нижче»), а `✓`/`⚠` уникались саме як оцінка. Дві поверхні
> розійшлись — застосунок від S9 говорив інакше, — і оператор звів їх, обравши мову
> застосунку. Рішення **людське**; зафіксоване також у `web/app.js`, щоб його не
> «полагодили» назад як розбіжність.
>
> **Попередня мапа (2026-07-17 — 2026-07-26), для історії:**
> `verified` → «↓ Нижче за 30 днів» / «нижче нашого мінімуму»;
> `verified_provisional` → «↓ Нижче · мало даних»; `pumped` → «≥ Не нижче» /
> «не нижче нашого мінімуму»; `declared` → «Ще не перевірено»;
> `insufficient_history` → «історії ще мало».
>
> **Що змінилось по суті, а не в словах:**
> 1. `✓`/`⚠` і слово «перевірена» ближчі до оцінки, ніж «↓»/«≥». Аргумент §7.5.1 («ми не
>    знаємо статутного мінімуму, лише свої виміри») **не знято** — його тепер несе не
>    пігулка, а провенанс на сторінці товару: «за нашими спостереженнями ціна за 30 днів
>    не була нижчою за X ₴ · основа: N вимірів у M з K діб».
> 2. Для `declared` і `insufficient_history` **не кажемо нічого**. Раніше картка казала
>    «заявлено · ще не перевірено». Мовчання коротше, але воно НЕ нейтральне: відсутність
>    мітки читається як «претензій нема». Заміряно наслідок: у стрічці мітку мають 48 із
>    50 карток (сорт «наше зниження» піднімає перевірені), але на вітрині головної було
>    **0 із 6** — саме тому вітрину переведено на перевірені (`tasks/S21`).
