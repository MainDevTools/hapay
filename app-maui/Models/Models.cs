...
public class Discount
{
    ...
    /// Мікрографік 30 днів: [[зсув доби від початку вікна, копійки], …]. Сервер віддає
    /// його з S32 і НЕ кладе сюди товари з надто короткою історією (менше трьох діб) —
    /// тобто порожній масив тут означає «нам ще нема чого показати», а не «дані є, але
    /// не влізли». Малює SparkDrawable.
    [JsonPropertyName("spark")] public List<List<long>>? Spark { get; set; }
    ...
    [JsonIgnore] public bool HasSpark => Spark is { Count: >= 3 };
    /// Гліф розділу показуємо ЛИШЕ там, де фото немає — інакше він світив би крізь
    /// прозорі PNG. Третина стрічки (KTC, Eldorado) приходить без фото взагалі.
    ...
    ///
    /// Поріг — 1% (рішення власника), той самий, що в детекторі (migrations/0012).
    /// Тут він потрібен окремо, бо картка рахує відсоток із СИРОГО price_snapshot,
    /// а не з discount_event: у пошуку показуємо й товари без активної події.
    ///
    /// Порівнюємо ДО округлення — 0,75% округлилось би до «−1%» і виглядало б як
    /// чесна знижка (на проді таких було 20 із 27 підпорогових).
    ///
    /// AwayFromZero, а не типове для .NET банкірське: §5.1 вимагає ROUND_HALF_UP, і
    /// сервер (detection.core.pct) рахує саме так. Інакше 4,5% дало б тут 4, а там 5.
    [JsonIgnore]
    public int? Pct
    {
        get
